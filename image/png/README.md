...
see [atch-zlib.png](#atch-zlibpng) for more details.

The attachment contained in this PNG file is [`message.txt`](./message.txt). The
only difference from [atch-zlib.png](#atch-zlibpng) is that the attachment's
content in the `atCh` chunk is not compressed.

Source: own work, though simply derived from [atch-zlib.png](#atch-zlibpng)

Shell commands to generate:

```sh
curl -fsSLO https://github.com/skeeto/scratch/raw/58470254f4a95cdf7a53888e405c851c21eb2cae/pngattach/pngattach.c
cc -O3 -DNO_ZLIB -o pngattach pngattach.c
./pngattach -d message.txt < atch-zlib.png | ./pngattach -u message.txt > atch-plain.png
```
